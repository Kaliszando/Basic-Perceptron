...
static void fix_whitespace(std::string& str)
{
	for (int i = 0; i < str.size(); ++i)
	{
		if (str[i] != ' ') continue;

		std::string lhs = str.substr(0, i);
		std::string rhs = str.substr(i + 1, str.size() - i - 1);
    ...
    else
    {
                ...
                i != ls.end(); ++i)
        {
...
#endif
            {
				std::string str = *i;
				fix_whitespace(str);
                Gnuplot::m_sGNUPlotPath = str; // set m_sGNUPlotPath
                return true;
            }
        }
